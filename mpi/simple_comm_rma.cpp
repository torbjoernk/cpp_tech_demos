...
void doing_fine(ProcessData &data, const int iter) {
  ...
  if (!data.iam_first) {
    ...
    assert(mpi_err == MPI_SUCCESS);
    LOG(INFO) << "unlocked fine window to rank " << data.prev;
  }

  LOG(INFO) << "locking local fine window";
  mpi_err = MPI_Win_lock(MPI_LOCK_EXCLUSIVE, data.rank, 0, data.fine_win);
  assert(mpi_err == MPI_SUCCESS);

  LOG(INFO) << "start computation";
  data.fine_val_out = data.fine_val_in + (data.rank + 1) * FINE_MULTIPLIER;
  LOG(INFO) << data.fine_val_out << " = " << data.fine_val_in << " + " << ((data.rank + 1) * FINE_MULTIPLIER);
  ...
  LOG(INFO) << "done computation";

  mpi_err = MPI_Win_unlock(data.rank, data.fine_win);
  assert(mpi_err == MPI_SUCCESS);
  LOG(INFO) << "unlocked local fine window";
}
...
void doing_coarse(ProcessData &data, const int iter) {
  ...
  if (!data.iam_first) {
    LOG(INFO) << "locking coarse window to rank " << data.prev;
    mpi_err = MPI_Win_lock(MPI_LOCK_EXCLUSIVE, data.prev, 0, data.coarse_win);
    assert(mpi_err == MPI_SUCCESS);

    LOG(INFO) << "getting coarse value from " << data.prev;
    ...
    assert(mpi_err == MPI_SUCCESS);
    LOG(INFO) << "unlocked coarse window to rank " << data.prev;
  }

  LOG(INFO) << "locking local coarse window";
  mpi_err = MPI_Win_lock(MPI_LOCK_EXCLUSIVE, data.rank, 0, data.coarse_win);
  assert(mpi_err == MPI_SUCCESS);

  LOG(INFO) << "start computation";
  data.coarse_val_out = data.coarse_val_in + (data.rank + 1) * COARSE_MULTIPLIER;
  LOG(INFO) << data.coarse_val_out << " = " << data.coarse_val_in << " + " << ((data.rank + 1) * COARSE_MULTIPLIER);
  ...
  assert(mpi_err == MPI_SUCCESS);
  LOG(INFO) << "unlocked local coarse window";
}
...
void check_finished(ProcessData &data, const int iter) {
  ...
  if (!data.iam_first) {
    LOG(INFO) << "locking state window to rank " << data.prev;
    mpi_err = MPI_Win_lock(MPI_LOCK_EXCLUSIVE, data.prev, 0, data.state_win);
    assert(mpi_err == MPI_SUCCESS);

    LOG(INFO) << "getting state from " << data.prev;
    ...
    assert(mpi_err == MPI_SUCCESS);
    LOG(INFO) << "unlocked state window to rank " << data.prev;
  } else {
    other_state.finished = true;
  }

  LOG(INFO) << "locking local state window";
  ...
  assert(mpi_err == MPI_SUCCESS);
  LOG(INFO) << "unlocked local state window";
}
...
//                        iter   residual  coarse     fine
  ...
  do {
    ...
    myself.fine_val_in = 0;
    myself.coarse_val_in = 0;
